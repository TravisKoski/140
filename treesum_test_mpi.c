...
char *treesum_test1() {
  printf("%d/n", no_proc);
  int sum = global_sum(my_rank, my_rank, no_proc, comm);
  if(my_rank == 0){
  int expectedsum = (no_proc*(no_proc -1))/2;
  mu_assert("Failed global sum",sum == expectedsum );
  }
...
int main(int argc, char *argv[]) {
  ...
  if (my_rank == 0) {
    mu_print_test_summary("Summarys:");
  }
  MPI_Finalize();
  return 0;
}
