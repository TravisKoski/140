...
char *validate_vect(char *msgheader, double global_x[], int n, int t,
  ...
  for (i = 0; i < n; i++) {
    ...
    mu_assert("One mismtach in the multiplication", expected[i] == global_x[i]);
  }
  free(expected);
  return NULL;
}
...
int init_matrix(double *local_A, double *local_x, double *local_d,
  ...
  if (local_A == NULL || local_x == NULL || local_d == NULL ||
    ...
    for(int i = 0; i < n; i++){
      ...
      local_d[local(i, blocksize)] = (2.0*n-1)/n;
      }
    }
...
char *itmv_test1() {
  return itmv_test("Test 1", TEST_CORRECTNESS, 2, !UPPER_TRIANGULAR, 1);
}
